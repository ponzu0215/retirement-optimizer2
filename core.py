...
def build_lump_events(input_: Dict[str, Any], years_of_service: int, options: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    events.append({
        "age": int(input_["retirementAge"]),
        "items": [{"item": "退職一時金", "age": int(input_["retirementAge"]), "amount": safe_number(input_["severancePay"], 0.0)}],
        "amount": safe_number(input_["severancePay"], 0.0),
        "periods": [{"startAge": int(input_["retirementAge"]) - years_of_service, "endAge": int(input_["retirementAge"])}]
    })
    if options.get("dcMode") == "lump":
        ...
        events.append({
            ...
            "amount": amount,
            "periods": [{"startAge": int(input_["dcStartAge"]), "endAge": age}]
        })
    if options.get("idecoMode") == "lump":
        ...
        events.append({
            ...
            "amount": amount,
            "periods": [{"startAge": int(input_["idecoStartAge"]), "endAge": age}]
        })
...
def optimize_strategy(input_: Dict[str, Any], public_pension_annual: Number, years_of_service: int,
    ...
    if retire_age <= 55:
        fixed_age = retire_age + 20
        lump_age = max(60, min(fixed_age, max_lump_age))
        dc_lump_ages = [lump_age]
        ideco_lump_ages = [lump_age]
    elif retire_age <= 59:
        dc_lump_ages = list(range(60, max_lump_age+1))
        ideco_lump_ages = list(range(60, max_lump_age+1))
    else:
        dc_lump_ages = [60]
        ideco_lump_ages = [60]

    plus20_age = retire_age + 20
    can_plus20 = (plus20_age >= 60 and plus20_age <= max_lump_age)

    if retire_age <= 59:
        pension_start_ages = list(range(60, min(75, int(input_["endAge"])-1)+1))
    else:
    ...
    if pattern=="A":
        dc_candidates = ([retire_age, plus20_age] + dc_lump_ages) if can_plus20 else list(dc_lump_ages)
        ideco_candidates = ([retire_age, plus20_age] + ideco_lump_ages) if can_plus20 else list(ideco_lump_ages)
