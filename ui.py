...
def render_input_form(defaults: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
    ...
    with st.form("simulatorForm", clear_on_submit=False):
        ...
        with c4:
            serviceYears = st.text_input("勤続年数（年）", value=str(defaults.get("serviceYears", "")), key="serviceYears")
        ...
        c1, c2, c3, c4 = st.columns(4)
        with c1:
            dcStartAge = st.text_input("加入開始年齢", value=str(defaults.get("dcStartAge", "")), key="dcStartAge")
        with c2:
            dcEndAge = st.text_input("拠出終了年齢", value=str(defaults.get("dcEndAge", "")), key="dcEndAge")
        with c3:
        ...
        dcReturnRate_pct = st.text_input("想定年利率（%）", value=("" if str(defaults.get("dcReturnRate", "")).strip()=="" else str(float(defaults.get("dcReturnRate", 0.0))*100.0)), key="dcReturnRate_pct")
        st.markdown("</div>", unsafe_allow_html=True)
        ...
        with c2:
            idecoEndAge = st.text_input("拠出終了年齢 ", value=str(defaults.get("idecoEndAge", "")), key="idecoEndAge")
        with c3:
        ...
        idecoReturnRate_pct = st.text_input("想定年利率（%） ", value=("" if str(defaults.get("idecoReturnRate", "")).strip()=="" else str(float(defaults.get("idecoReturnRate", 0.0))*100.0)), key="idecoReturnRate_pct")
        st.markdown("</div>", unsafe_allow_html=True)

        st.markdown('<div class="mz-section"><h2>💵 給与・年金情報</h2>', unsafe_allow_html=True)
        c1, c2 = st.columns(2)
        with c1:
            currentSalary = st.text_input("現在の年収（万円）", value=str(defaults.get("currentSalary", "")), key="currentSalary")
        with c2:
            st.markdown(
                '''
                <div class="avg-salary-label">
                  <span>平均標準報酬月額（万円）</span>
                  <span class="avg-salary-tooltip" title="入社～退職までの平均年収による平均標準報酬月額の概算
・年収400万円：約35万円
・年収500万円：約44万円
・年収600万円：約53万円
・年収700万円：約60万円
・年収800万円以上：約65万円">
        ...
        endAge = st.text_input("計算終了年齢（受給終了年齢）", value=str(defaults.get("endAge", "")), key="endAge")
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    input_internal = {
        ...
        "retirementAge": _parse_int(retirementAge),
        "joinAge": _parse_int(joinAge),
        "serviceYears": _parse_int(serviceYears),
        "severancePay": _parse_float(severancePay),

        "dcStartAge": _parse_int(dcStartAge),
        "dcEndAge": _parse_int(dcEndAge),
        ...
        "idecoEndAge": _parse_int(idecoEndAge),
